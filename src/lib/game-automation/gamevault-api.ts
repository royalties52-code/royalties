...
export class GameVaultApiClient {
  ...
  constructor(config: GameVaultApiConfig = {}) {
    ...
    this.agentId = (config.agentId || process.env.GAMEVAULT_AGENT_ID || "").trim();
    this.secretKey = (config.secretKey || process.env.GAMEVAULT_SECRET_KEY || "").trim();
    this.proxyUrl = (config.proxyUrl || process.env.GAMEVAULT_PROXY_URL || "").trim() || undefined;
  }
